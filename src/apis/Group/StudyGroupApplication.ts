...
export const getGroupApplications = async ({
 ...
 const { data } = await axios.get<PagedResponse<Application>>(
  ...
  { params: { sort, order, page, size }, headers: getAuthHeader() }
 );
 return data;
};

// 신청 승인 (주최자) — 응답 바디: Application 1건
...
// 신청 거절 (주최자) — 응답 바디: Application 1건
...
export const getMyApplications = async (opts?: {
 ...
 const { data } = await axios.get<PagedResponse<Application>>(
  ...
  { params: { sort, page, size }, headers: getAuthHeader() }
 );
 return data;
};
