...
export async function updateMe(payload: UpdateMePayload) {
  ...
  if (payload.password && payload.password.trim()) fd.append("password", payload.password);
  if (payload.profileImage instanceof File) fd.append("profileImage", payload.profileImage);
  if (payload.birthday) fd.append("birthday", payload.birthday);

  const { data } = await api.patch<UpdateMeResponse>("/api/users/me", fd, {
